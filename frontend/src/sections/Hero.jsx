...
export default function Hero() {
  ...
  return (
    <section id="top" className="relative min-h-[100svh] w-full overflow-hidden" data-testid="hero">
      {/* 3D background */}
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "radial-gradient(ellipse at center, transparent 40%, rgba(3,3,3,0.85) 100%)",
      }} />
      ...
      {/* Content */}
      <div ref={wrap} className="relative z-10 mx-auto max-w-7xl px-6 md:px-10 pt-40 pb-24 min-h-[100svh] flex flex-col justify-center">
        <motion.div
        ...
        <h1 className="mt-8 font-display tracking-tighter leading-[0.95] text-[14vw] md:text-[10vw] lg:text-[8.4rem] xl:text-[9.5rem]" data-testid="hero-headline">
          {headingLines.map((line, li) => (
            <div key={li} className="overflow-hidden">
              ...
              >
                {line.map((w, wi) => (
                  <span key={wi} className={wi === 1 || (li === 2) ? "gold-text-gradient italic font-light" : "text-[var(--apex-warm)]"}>
                    {w}
                  </span>
                ))}
              </motion.div>
        ...
        <motion.p
          ...
          className="mt-10 max-w-xl text-base md:text-lg text-white/65 leading-relaxed"
        >
        ...
        >
          <a href="#contact" data-magnetic data-testid="hero-cta-primary" className="magnetic-btn primary">
            <span className="btn-fill" />
            Book Free Consultation
            <span aria-hidden>→</span>
          </a>
          <a href="#portfolio" data-magnetic data-testid="hero-cta-secondary" className="magnetic-btn">
            <span className="btn-fill" />
            Explore Our Work
          </a>
        </motion.div>
    ...
    <div className="absolute inset-0 pointer-events-none hidden lg:block">
      {cards.map((c, i) => (
        <motion.div
          ...
          className={`absolute ${c.pos} floating`}
  ...
  return (
    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2" data-testid="scroll-indicator">
      ...
      <div className="relative h-14 w-px bg-white/15 overflow-hidden">
        <motion.div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[var(--apex-gold)]"
